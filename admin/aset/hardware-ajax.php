...
if(isset($_POST['kategori'])){
	 ...
	 $showid =mysqli_fetch_array(mysqli_query($conn,"SELECT id FROM status_labels WHERE name= '$status'"));
	 $idstatus=$showid['id'];
		 $sql1 ="SELECT data_aset.*,data_aset.nik as tnik,data_aset.kd_uker as tuker, data_karyawan.*,data_uker.*,data_uker_bagian.* 
				...
				WHERE data_aset.status='$idstatus'";
			$sql2="SELECT data_aset.*,data_aset.nik as tnik,data_aset.kd_uker as tuker, data_karyawan.*,data_uker.*,data_uker_bagian.* 
					FROM data_aset left join data_karyawan ON data_aset.nik=data_karyawan.nik
					Left join data_uker ON data_aset.kd_uker=data_uker.kd_uker
					Left join data_uker_bagian ON data_aset.kd_uker=data_uker_bagian.kd_bag
					WHERE data_aset.status='$idstatus'  AND";
			$sql3="data_aset.status='$idstatus' AND";
