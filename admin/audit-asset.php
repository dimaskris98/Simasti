...
if (isset($_POST['submit'])) { 
	...
	$nonorganikdep=mysqli_fetch_array(mysqli_query($conn,"Select sum(formasi_non_organik) AS totalnonorganikdep From data_uker where kd_uker='$kduker'"));  
	$shownonorganikbag=mysqli_fetch_array(mysqli_query($conn,"Select SUM(non_organik_bag) AS totalnonorganikbag From data_uker_bagian where kd_uker='$kduker'")); 
	$nonorganikbag=$shownonorganikbag['totalnonorganikbag'];
	 
	 $totalnonorganik=$shownonorganikbag['totalnonorganikbag'] +$nonorganikdep['totalnonorganikdep'];
	...
	where data_uker.kd_uker='$kduker'");
		// echo  "Select SUM(non_organik_bag) AS totalnonorganikbag From data_uker_bagian where kd_uker='$kduker'";
		...
		<table >
			<thead bgcolor="silver">
				<tr>
					...
					<th rowspan="2">Total Formasi</th>
					<th colspan="8">Grade</th>
					<th rowspan="2">Terisi</th><th rowspan="2">Non Organik</th>
				</tr>
				<tr>
		...
		<table>
			<thead bgcolor="silver">
				<tr >  
					...
					<th rowspan="3">Organik</th> 
					<th rowspan="3">Non Organik</th> 
					<th colspan="8">Jumlah Aset</th> <th colspan="7">Kebutuhan</th> 
				</tr> 
				<tr>
					<?php
			...
			<tbody>
				...
				echo '<tr>
						...
						<td align="center" >'.$ker['formasi_non_organik'].'</td>';
						$res = $conn->query("SELECT * FROM data_kategori ");
						while($row = $res->fetch_assoc())
						{ 
							$kdkat=$row['kd_kategori'];
						...
						{ 
							...
							?>
						<td align="center" contenteditable="true" onBlur="saveToDatabase(this,'qty','<?php echo $showkebutuhan['id']; ?>')" onClick="showEdit(this);"><?php echo $showkebutuhan['qty']; ?></td>
				...
				$kdbag=$rowbag['kd_bag'];
				$showTotal =mysqli_fetch_array(mysqli_query($conn,"SELECT *, SUM(non_organik_bag) as nonorganikbagTOTAL FROM  data_uker_bagian where kd_uker='$kduker'"));
				$totalasetbagian = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM data_aset WHERE kd_uker = '$kdbag'"));
					...
					<td align="center">'.$showTotal['nonorganikbagTOTAL'].'</td>';
							$total=0;
							$res = $conn->query("SELECT * FROM data_kategori ");
							while($row = $res->fetch_assoc())
							{ 
							...
							{ 
								$kdkat=$row['kd_kategori'];
							$showmodel =mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM  kebutuhan where id_uker='$kdbag' AND id_kategori='$kdkat'"));
							 ?>
						<td align="center" contenteditable="true" onBlur="saveToDatabase(this,'qty','<?php echo $showmodel['id']; ?>')" onClick="showEdit(this);"><?php echo $showmodel['qty']; ?></td>
				...
				echo'<tr bgcolor="silver"><td align="right">Sub Total<br></td><td align="center">'.$totalformasi.'<br></td><td align="center">'.$totalnonorganik.'<br></td>';
				$totalall=0;
