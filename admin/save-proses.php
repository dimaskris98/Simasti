...
if (isset($_POST["nik"]) && strlen($_POST["nik"]) > 0) {
	...
	$sql = "INSERT INTO data_karyawan  VALUES ('$nik', '$nama', '$kd_bag', '$bagian', '$departemen','','','','$tipe')";

	if (mysqli_query($conn, $sql)) {
		//$my_id = mysqli_insert_id($conn);
		echo '<option value="' . $nik .'/'.$nama. '" selected="selected">' . $nik . ' - ' . $nama . ' </option>';
	}
}
...
if (isset($_POST["konten_aset"]) && strlen($_POST["konten_aset"]) > 0) {
	...
	$sql = "INSERT INTO data_aset VALUES ('', '$aset', '$tahun', 'cm', '$model', '$sn', '', '', '', '', '', 
				'', '', '', '', '', '', '$ya', '', '', '', '', '', '', 
				'', '', '', '', '', '', '', '$id_user', '')";
