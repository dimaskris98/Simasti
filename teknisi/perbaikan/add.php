		...
		if(isset($_POST['save']))
			 {
				...
				$status="open"; 
				 $id_user=$_POST['teknisi'];
				   
				$sql = "INSERT INTO perbaikan  VALUES ('', '$no_aset', '$pengirim', 
				'$tlp', '$keluhan', '$tgl_datang', '',  NULL, '$status','$sewa', '$id_user')";
	...
	<div class="col-md-7">
			...
			{ 
				 ...
				 <select class="form-control select2" name="no_aset_id" id="no_aset_id"  required  >
										 <option value=""></option>'.
									<?php	 
											$res = $conn->query("SELECT * FROM data_aset where sewa='$sewa' AND lokasi='DI USER'");
											while($row = $res->fetch_assoc()){
												echo '
													<option value="'.$row['no'].'"> '.$row['no_aset'].' - '.$row['model'].' - '.$row['kd_kategori']. ' </option>
													';
...
?>
							...
							<div class="form-group ">
								...
								<div class="col-md-7 col-sm-12 required">
									<input class="form-control" type="text" name="pengirim" id="pengirim" value=" " required /> 
								</div>
							</div>  
							<!-- tlp -->
							<div class="form-group ">
							...
							<div class="form-group ">
								...
								<div class="col-md-7 col-sm-12 required">
									<textarea class="form-control" type="number" name="keluhan" id="keluhan"> </textarea>
								</div>
							</div>
							<div class="form-group ">
							...
							<div class="form-group ">
								<label for="name" class="col-md-3 control-label">Teknisi</label>
								<div class="col-md-9">
									<div class="input-group col-md-5"   >
										 <select class="form-control select2" name="teknisi" id="teknisi"  required  >
										 <option value=""></option>'.
									<?php	 
											$res = $conn->query("SELECT * FROM tb_teknisi left join users ON tb_teknisi.id_user=users.id_user ");
											while($row = $res->fetch_assoc()){
												echo '
